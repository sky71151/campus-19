...
/*   Updated: 2023/11/08 08:13:29 by rvan-bae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map_valid(char *filename, board_settings *settings)
{
	...
	printf("%d %d %c %c %c\n", settings->rows, settings->line_lenght,settings->empty, settings->obstacle, settings->fill);
...
int	check_first_line(char *file, board_settings *settings)
{
	...
	if (index >= 4)
	{
		...
		//printf("%d %d %c %c %c\n", set.rows, set.line_lenght,set.empty, set.obstacle, set.fill);
	}
	else
	{
		free(line);
